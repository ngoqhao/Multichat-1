...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        // ── Mở Swing GUI nếu có màn hình (local), bỏ qua nếu headless (Render) ──
        boolean hasDisplay = !java.awt.GraphicsEnvironment.isHeadless();
        if (hasDisplay) {
            javax.swing.SwingUtilities.invokeLater(() -> {
                ChatServer gui = new ChatServer(port);
                gui.setVisible(true);
            });
        } else {
            System.out.println("ℹ️  Headless mode — dùng /admin.html để quản lý");
        }
